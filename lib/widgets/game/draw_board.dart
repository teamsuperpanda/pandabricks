...
final Paint _sparkleCorePaint = Paint();
final Paint _sparkleWhitePaint = Paint();
final Paint _rowFlashPaint = Paint();

// Hoisted gradient used for the per-cell inner highlight.
const LinearGradient _innerHighlightGradient = LinearGradient(
  colors: [Colors.white54, Colors.transparent],
  begin: Alignment.topLeft,
  end: Alignment.bottomRight,
);

Map<int, LinearGradient> buildShaderGradients(List<Color> palette) {
...
TextPainter cachedEmojiPainter({
...
void drawBoard(
  ...
  for (final cell in filledCells) {
    ...
    _cellPaint
      ..maskFilter = null
      ..shader = fillShaders[idx] ??= gradients[idx]!.createShader(fillRect);
    canvas.drawRRect(fillRRect, _cellPaint);
  ...
  for (final eff in effects) {
    if (eff.type == EffectType.rowFlash) {
      // Fading white flash over a cleared row. Alpha peaks near 0.5 and
      // fades out over the effect duration, mirroring the sparkle curve.
      final t = 1.0 - (eff.alpha / 160.0);
      final alpha = (0.5 * (1 - t)).clamp(0.0, 0.5);
      _rowFlashPaint.color = Colors.white.withValues(alpha: alpha);
      canvas.drawRect(
        Rect.fromLTWH(
          offsetX,
          offsetY + eff.y * cellSize,
          boardWidth,
          cellSize,
        ),
        _rowFlashPaint,
