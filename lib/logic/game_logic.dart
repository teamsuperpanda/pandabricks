...
import 'package:pandabricks/logic/modes/modes.dart'; // Import the modes
...
class GameLogic {
    ...
    GameLogic(this.mode)
        : playfield = List.generate(20, (_) => List.filled(10, 0)), // 20 rows, 10 columns
          score = 0,
          level = 1;
    ...
    bool shouldSpawnPandabrick() {
        ...
        return chance < mode.pandabrickSpawnPercentage; // Check against the percentage
    }

    bool shouldSpawnSpecialBlock() {
        // Logic to determine if a special block should spawn in Bamboo Blitz mode
        int chance = (DateTime.now().millisecondsSinceEpoch % 100);
        return chance < mode.specialBlocksSpawnPercentage; // Check against the percentage
    }
